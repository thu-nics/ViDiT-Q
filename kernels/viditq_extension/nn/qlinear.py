...
class W8A8OF16LinearDynamicInputScale(nn.Module):
    ...
    def __from_liner__(
        ...
        assert (
            linear.weight.dtype == torch.float16
        ), f"weight type!=torch.float16, is {linear.weight.dtype}"
...
class W8A8OBF16LinearDynamicInputScale(nn.Module):
    ...
    def __from_liner__(
        ...
        ), f"weight type!=torch.bfloat16, is {linear.weight.dtype}"
        ...
        if weight_sym:
            ...
            self.weight.data[:, :] = weight_quant.to(torch.int8).contiguous()
            self.scale_weight.data[:] = weight_scale.to(torch.bfloat16).contiguous()
    ...
    def forward(
        ...
        if bias is None:
            bias = torch.zeros(
                self.out_features, dtype=torch.bfloat16, device=input.device
            )
        ...
        if self.zp_weight is not None:
            output = qgemm.w8a8_bf16_bias_weight_asym(
                input_q,
                self.weight,
                bias,
                quant_params.scale_input,
            ...
            output = qgemm.w8a8_bf16_bias_weight_sym(
                input_q,
                self.weight,
                bias,
                quant_params.scale_input,
